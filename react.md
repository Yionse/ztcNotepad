...
- 在同一个位置渲染两个相同的组件，这两个相同的组件共用同一个state，那么这两个组件会被视为同一个组件，就算让这两个组件轮流销毁，state也不会更新，这是因为，在react看来，他们两个组件渲染在了同一个地方，它们就是一个组件，于是不会被销毁，所以也就导致state不会被更新
...
- 注意使用useEffect发送网络请求时，不要直接写在useEffect的参数函数上，会导致垃圾清理问题，争取的做法是，在匿名函数内，重新再定义一个具名函数
...
- 是一个允许你在多次渲染中缓存函数的hook

- 接收两个参数，第一个参数是一个函数，第二个参数是一个数组，这个数组是一个依赖项，当依赖项中的内容发生改变时，才会调用里面的函数，否则不会调用，起到优化的操作
...
- ```tsx
  ...
  - 举例：当productId为1时，返回的调用接口名称是**/product/1/buy**，而只要后面依赖项中的两个变量一直没有更改说明，函数的请求就没有发生改变，所以可以不用重新调用传入的函数
  - 当productID为2时，已经发生了更改，如果再调用刚才的接口名称，显然已经不合适了，所以需要重新调用一下函数，返回一个新的接口名称，然后再进行调用
  - 第二个参数依赖项，当里面填入的参数发生改变时，就会让函数重新触发，返回新的接口名


## memo

- 使用memo函数将组件包裹起来，在props没有改变的情况下，跳过重新渲染

- 记忆化是一种性能优化，而非保证

- 要记忆化一个组件，将它包裹在原来的组件外面，使用它的返回值替换原来的组件

- ```tsx
  const Component = memo(() => {
      return (<div>这是一个记忆化组件</div>);
  });
  export default Component
  ```

- 注意当memo的自身state发生变化时，是会重新渲染组件的，只有当父组件传过来的Props没有发生改变才会触发优化
...
- 一种基于hooks的范式的简易数据管理方案，通常用于全局共享数据

- 启用方式：当src目录下，存在model目录则启用，每个model文件都默认都出一个函数
...
- 在model内部定义的状态看上去像是在局部定义的，实际上，他们共享的是同一个状态，类似于Vue中的Vuex

- 它是@umijs/plugin-model包下的一个hooks
